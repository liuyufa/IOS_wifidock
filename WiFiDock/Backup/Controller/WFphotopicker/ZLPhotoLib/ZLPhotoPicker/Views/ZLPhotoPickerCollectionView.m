...
#define MAX_COUNT 1024 // 选择图片最大数默认是9
...
- (void)selectAllFile:(BOOL)flag
{
    if (self.selectAsstes.count == self.dataArray.count) {
        
        for (int row = 0; row < self.dataArray.count; row++) {
            NSIndexPath *index = [NSIndexPath indexPathForRow:row inSection:0];
     
            ZLPhotoPickerCollectionViewCell *cell = (ZLPhotoPickerCollectionViewCell *)
            [self cellForItemAtIndexPath:index];
            ...
            [self.selectAsstes removeAllObjects];
        ...
        for (int row = 0; row < self.dataArray.count; row++) {
            ...
            ZLPhotoAssets *asset = self.dataArray[index.row];
            
            if ([self.selectAsstes containsObject:asset]) {
                ...
                if (pickerImageView.isMaskViewFlag) {
                    ...
                    [self.selectsIndexPath addObject:@(index.row)];
