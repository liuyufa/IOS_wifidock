...
//    self.navigationItem.rightBarButtonItem
    UIBarButtonItem *btn1 = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(back)];
...
//    [self.collectionView.selectAsstes removeAllObjects];
    [self.collectionView selectAllFile:self.selectFlag];
...
- (void)photoBrowser:(ZLPhotoPickerBrowserViewController *)photoBrowser removePhotoAtIndex:(NSUInteger)index{
    ...
    NSLog(@"self.makeView.text = %@",self.makeView.text);
}
...
- (void) done
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KSelectePhotos object:nil userInfo:@{@"selectAssets":self.collectionView.selectAsstes}];
