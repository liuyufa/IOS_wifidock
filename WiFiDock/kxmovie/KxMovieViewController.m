...
{    
    ...
    if ([path rangeOfString:LOCAL_HTTP_PATH].location == NSNotFound) {
        isSmb = FALSE;
    }else
        isSmb = TRUE;
    if ([WFFileUtil isAudio:[path pathExtension]]) {
        isAudio = TRUE;
    }else
        isAudio = FALSE;
    isChangeTrack = FALSE;
...
+ (id)movieViewControllerWithContent:(NSArray*)query current:(NSString *)path parameters: (NSDictionary *) parameters withIndex:(NSInteger)index
{
    itemPath = path;
    itemList = query;
    currentItemIndex = index;
    totalItemIndex = [itemList count];
    if ([path rangeOfString:LOCAL_HTTP_PATH].location == NSNotFound) {
        isSmb = FALSE;
    }else
        isSmb = TRUE;
...
-(void)viewDidLoad{
    NSLog(@"self.view.frame.width = %f self.view.frame.height = %f ",self.view.frame.size.width,self.view.frame.size.height);
    NSLog(@"_topHUD.frame.size.width = %f,_topHUD.frame.size.height = %f",_topHUD.frame.size.width,_topHUD.frame.size.height);
}
...
- (void)loadView
{
    ...
    _topHUD.frame = CGRectMake(0,0,width,44);//30 - 40
    NSLog(@"width = %f",width);
    ...
    _doneButton.frame = CGRectMake(0,4,50,34);//24 - 34
    _doneButton.backgroundColor = [UIColor clearColor];
    [_doneButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_doneButton setTitle:NSLocalizedString(@"Done", nil) forState:UIControlStateNormal];
    _doneButton.titleLabel.font = [UIFont systemFontOfSize:15];//12 - 15
    */
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    //_doneButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    _doneButton.frame = CGRectMake(5,10,54,34);//24 - 34
    _doneButton.backgroundColor = [UIColor clearColor];
    //_doneButton.currentTitleShadowColor = [UIColor lightGrayColor];
    //[_doneButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_doneButton setTitle:NSLocalizedString(@"Done", nil) forState:UIControlStateNormal];
    _doneButton.titleLabel.font = [UIFont systemFontOfSize:15];//12 - 15
    ...
    _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(58,15,50,20)];//5 - 15
    ...
    _progressSlider = [[UISlider alloc] initWithFrame:CGRectMake(110,14,width-192,20)];//4 - 14
    ...
    _leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(width-80,15,60,20)];//5 -15
    ...
    _infoButton.frame = CGRectMake(width-33,10,30,30); //5 - 10 20 - 25
    ...
    if (totalItemIndex > 1) {
        [_rewindButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_prev"] forState:UIControlStateNormal];
    }
    else
        [_rewindButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_rew"] forState:UIControlStateNormal]; //playback_prev
    ...
    if (totalItemIndex > 1) {
        [_forwardButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_next"] forState:UIControlStateNormal];
    }
    else
        [_forwardButton setImage:[UIImage imageNamed:@"kxmovie.bundle/playback_ff"] forState:UIControlStateNormal]; //playback_next
...
{    
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    [[UIApplication sharedApplication] endReceivingRemoteControlEvents]; // 让后台可以处理多媒体的事件
...
-(void) playEnd{
    if (totalItemIndex > 1) {
        if(currentItemIndex < (totalItemIndex - 1)){
            [self pause];
            [self freeBufferedFrames];
            [_decoder closeFile];
            _moviePosition = 0;
            //[self play];
            currentItemIndex++;
            itemPath = [itemList objectAtIndex:currentItemIndex];
            if(isSmb){
                WFFile *item = (WFFile*)itemPath;
                NSString *path = [NSString stringWithFormat:@"%@:%d%@",LOCAL_HTTP_PATH,LOCAL_HTTP_PORT,[item.filePath stringByReplacingOccurrencesOfString:SAMBA_URL withString:@""]];
                path = [path stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
                [_decoder openFile:path error:nil];
                //[self initWithContentPath: path parameters: _parameters];
            }else{
                WFFile *item = (WFFile*)itemPath;
    ...
    if (totalItemIndex > 1) {
        if(currentItemIndex < (totalItemIndex - 1)){
            ...
            if(isSmb){
                ...
                NSString *path = [NSString stringWithFormat:@"%@:%d%@",LOCAL_HTTP_PATH,LOCAL_HTTP_PORT,[filePath stringByReplacingOccurrencesOfString:SAMBA_URL withString:@""]];
...
//                LocalFileItem *item = (LocalFileItem*)itemPath;
//                NSString *path = item.context.filePath;
//                [_decoder openFile:path error:nil];
                //[self initWithContentPath: path parameters: _parameters];
            }
            [self play];
...
- (void)changeTrackTitles
{
    ...
    NSDictionary *dt = _decoder.info[@"metadata"];
...
//    int datalen;
    ...
    NSArray *allKeys = [dt allKeys];
    for (NSString *str in allKeys) {
        [d setObject:[dt valueForKey:str] forKey:[str lowercaseString]];
       
    }

    
    NSString *title = [[d valueForKey:@"title"]stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSString *album = [[d valueForKey:@"album"]stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSString *artist = [[d valueForKey:@"artist"]stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
/*
    unichar a = 0;
    if (title != nil) {
        a = [title characterAtIndex:0];
        for (<#initialization#>; <#condition#>; <#increment#>) {
            <#statements#>
        }
    }else if(artist != nil) {
        a = [artist characterAtIndex:0];
    }else if(album != nil) {
        a = [album characterAtIndex:0];
    }
        
    if (a == 0) {
    }
    else if( a > 0x4dff && a < 0x9fff){
        isUTF8 = TRUE;
    }else{
        len = (title.length > 100)?100:title.length;
        
        for (int i=0; i<len; i++) {
            unichar ch = [title characterAtIndex:i];
            //temp[i] = [ch characterAtIndex:1];
            int value = ch - 0;
            temp[i] = value;
        }
        titleData = [[NSData alloc]initWithBytes:temp length:len];
        
        
        len = (album.length > 4)?4:album.length;
        for (int i=0; i<len; i++) {
            unichar ch = [album characterAtIndex:i];
            //temp[i] = [ch characterAtIndex:1];
            int value = ch - 0;
            temp[i] = value;
        }
        albumData = [[NSData alloc]initWithBytes:temp length:len];
        
       
        len = (artist.length > 100)?100:artist.length;
        for (int i=0; i<len; i++) {
            unichar ch = [artist characterAtIndex:i];
            //temp[i] = [ch characterAtIndex:1];
            int value = ch - 0;
            temp[i] = value;
        }
        artData = [[NSData alloc]initWithBytes:temp length:len];
    }
    ...
    if (playingInfoCenter)
    {
        ...
        if (title != nil) {
            ...
            for (int i=0; i<title.length; i++) {
                a = [title characterAtIndex:i];
                if( a > 0x4dff && a < 0x9fff){
                    isUTF8 = TRUE;
                    break;
                }
            }
            if (isUTF8) {
                ...
                titleData = [[NSData alloc]initWithBytes:temp length:len];
                [songInfo setObject:[[NSString alloc] initWithData:titleData encoding:enc] forKey:MPMediaItemPropertyTitle];
            }
        }
        ...
        if(album != nil) {
            ...
            if (isUTF8) {
                ...
                len = (album.length > 100)?100:album.length;
                ...
                albumData = [[NSData alloc]initWithBytes:temp length:len];
                [songInfo setObject:[[NSString alloc] initWithData:albumData encoding:enc] forKey:MPMediaItemPropertyAlbumTitle];
            }
        }
        ...
        if(artist != nil) {
            ...
            if (isUTF8) {
                ...
                artData = [[NSData alloc]initWithBytes:temp length:len];
                [songInfo setObject:[[NSString alloc] initWithData:artData encoding:enc] forKey:MPMediaItemPropertyArtist];
            }
        }
        /*
        if (a == 0) {
        ...
        */
        UIImage *image = [self.artworkFrame asImage];
        if (image != nil) {
            albumArt = [[MPMediaItemArtwork alloc] initWithImage:image];
            [songInfo setObject:albumArt forKey:MPMediaItemPropertyArtwork];
        }
        //duration
        //NSLog(@"%d",(int)_decoder.duration);
        [songInfo setObject:[NSNumber numberWithInt:(int)_decoder.duration] forKey:MPMediaItemPropertyPlaybackDuration];
...
- (void) tick
{
    ...
    //test
    if (_decoder.validVideo &&((_tickCounter % 300) == 0)) {
        [self showHUD:NO];
    }
}
...
- (CGFloat) presentFrame
{
    ...
    if (_decoder.validVideo) {
        ...
        if (self.artworkFrame) {
            ...
            [self changeTrackTitles];//zhaoyu-0721
            isChangeTrack = TRUE;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == KxMovieInfoSectionGeneral) {
        ...
        NSString* detail = [d valueForKey:key];
        
        unichar a = [detail characterAtIndex:0];
        if( a > 0x4dff && a < 0x9fff){
            cell.detailTextLabel.text = detail;
        }
        else{
            int len = detail.length;
            Byte *temp = (Byte *)malloc(len + 1);
            for (int i=0; i<len; i++) {
                unichar ch = [detail characterAtIndex:i];
                //temp[i] = [ch characterAtIndex:1];
                int value = ch - 0;
                temp[i] = value;
            }
            
            NSData *data = [[NSData alloc]initWithBytes:temp length:detail.length];
            
            NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
            NSString *retStr = [[NSString alloc] initWithData:data encoding:enc];
            cell.detailTextLabel.text = retStr;
            //[str stringByReplacingPercentEscapesUsingEncoding:enc];
            free(temp);
        }
